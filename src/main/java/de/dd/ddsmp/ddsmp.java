...
public final class ddsmp extends JavaPlugin implements Listener, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("chunk")) {
            if (args.length == 0) { p.sendMessage(prefix + "§dVerwendung: §e/chunk <claim | unclaim | trust | untrust | info>"); return true; }
            switch (args[0].toLowerCase()) {
                ...
                default: p.sendMessage(prefix + "§cUnbekanntes Argument!"); return true;
            }
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (!command.getName().equalsIgnoreCase("chunk")) return Collections.emptyList();
        List<String> completions = new ArrayList<>();
        if (args.length == 1) completions.addAll(Arrays.asList("claim","unclaim","trust","untrust","info"));
        return completions;
    }
